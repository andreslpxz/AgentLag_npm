...
export const searchInFiles = tool(
  searchInFilesImpl,
  {
    ...
    schema: z.object({
      ...
      maxResults: z.number().int().min(1).max(200).optional().default(50).describe("Máximo de resultados"),
    }),
  }
);
...
export const applyPatchTool = tool(
  ...
  },
  {
    ...
    schema: z.object({
      ...
      timeoutMs: z.number().int().min(1).max(MAX_SHELL_TIMEOUT_MS).optional().default(15000).describe("Timeout en milisegundos"),
    }),
  }
);
...
export const runShell = tool(
  ...
  },
  {
    ...
    schema: z.object({
      ...
      timeoutMs: z.number().int().min(1).max(MAX_SHELL_TIMEOUT_MS).optional().default(DEFAULT_SHELL_TIMEOUT_MS).describe("Timeout configurable en ms (máx. 600000)."),
    }),
  }
);
...
export const manageMemory = tool(
  ...
  },
  {
    ...
    schema: z.object({
      ...
      ttlDays: z.number().min(1).optional().describe("Días hasta expirar la entrada."),
    }),
  }
);
